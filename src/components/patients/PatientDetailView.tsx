...
import { usePatient, usePatientDetailResources } from "@/lib/query";
...
export function PatientDetailView({ patientId }: { patientId: string }) {
  ...
  const {
    ...
    failedSections,
  } = usePatientDetailResources(patientId);
  ...
  const isCadavericDonor =
    patient.extension?.some((x) => x.url === EXT_CADAVERIC_DONOR && x.valueBoolean === true) ?? false;
  ...
  const birthPlaceDisplay =
    [birthPlaceExt?.valueAddress?.city, birthPlaceExt?.valueAddress?.country]
      .filter(Boolean)
      .join(", ") || birthPlaceExt?.valueAddress?.text;
  ...
  const emergencyContacts = relatedPersons.filter((r) =>
    r.relationship?.some((rel) => rel.coding?.some((c) => EMERGENCY_REL_CODES.has(c.code ?? ""))),
  );
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <StartEncounterButton patientId={patientId} patient={patient} />
          <RecordVitalsButton
            patientId={patientId}
            encounters={enc.filter((e) => e.status === "in-progress")}
          />
          <Link
          ...
          <Link
            href={`/patients/${patientId}/facesheet`}
            className={cn(buttonVariants({ variant: "outline" }), "inline-flex items-center gap-2")}
          >
            ...
            <div className="flex-1 min-w-0 space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                {mrn ? (
                  ...
                  <span className="inline-flex items-center text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-md px-2 py-0.5">
                    No MRN
                  </span>
                )}
                {patient.gender && (
                ...
                {criticalDirectives.map((d) => {
                  ...
                  return (
                    <span
                      key={d.id}
                      className="inline-flex items-center gap-1 text-xs font-bold text-white bg-red-600 border border-red-700 rounded-md px-2 py-0.5"
                    >
            ...
            {activeEncounter ? (
              ...
              >
                ...
                {(() => {
                  ...
                  return code ? (
                    <StatusPill
                      color={triageAcuityColor(code)}
                      label={triageAcuityLabel(code)}
...
function SignalChip({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium ${signalStyles[variant]}`}
    >
