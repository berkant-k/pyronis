...
export function DashboardView() {
  ...
  const patientsQuery = useRecentPatients(8);
  const encountersQuery = useRecentEncounters(6);
  ...
  const failedSections = [
    countsQuery.isError && "Statistics",
    patientsQuery.isError && "Recent patients",
    encountersQuery.isError && "Recent encounters",
  ].filter(Boolean) as string[];

  const isInitialLoading =
    countsQuery.isLoading && patientsQuery.isLoading && encountersQuery.isLoading;
