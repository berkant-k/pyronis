...
import { searchReferrals } from "@/lib/fhir-client"

export const dynamic = "force-dynamic"
export const metadata: Metadata = { title: "Referrals | Pyronis EMR" }

export default async function ReferralsPage() {
  const result = await searchReferrals({ count: 60 }).catch(() => null)
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Specialist referrals across all patients</p>
        </div>
      </div>
      {fetchFailed && (
      ...
      <ReferralsSearch initialData={initial} />
    </div>
  )
}
